...
const store = createStore({
    ...
    actions: {
        ...
        getAddBoardList(context) {
            ...
            .then(response => {
                ...
                if(data.length > 0 ) {
                    ...
                    //게시글 정보가 없을 때 처리
                    //더이상 게시글이 없기 때문에 서버에 요청을 하지 않기 위한 플래그 true 셋팅
                    context.commit('setNoMoreBoardList', true);
                    context.commit('setLastID', 0);
                    localStorage.removeItem('LsatID');
                }
        ...
        storeBoard(context, boardInfo) {
            ...
            const base64Payload = payload.replaceAll('-','+').replaceAll('_','/'); // base64로 된 페이로드 획득
            const objPayload = JSON.parse(window.atob(base64Payload)); // base64로 인코딩 된걸 다시 객체로 디코드
        ...
        register(context, formData) {
            ...
            .then(response => {
                console.log(response);
                // 사용자 정보를 상태에 저장
                context.commit('setUser', response.data); // response.data를 바로 사용
                context.commit('setAuthFlg', true);
